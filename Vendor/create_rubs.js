document.addEventListener('DOMContentLoaded', () => {
    ...
    const mockProperties = [
        { id: 'p1', name: '123 Main St', units: [{name: 'Unit 101', tenant: 'John Doe'}, {name: 'Unit 102', tenant: 'Jane Smith'}, {name: 'Unit 103', tenant: 'Bob Johnson'}] },
        { id: 'p2', name: '456 Oak Ave', units: [{name: 'Unit A', tenant: 'Alice Williams'}, {name: 'Unit B', tenant: 'Charlie Brown'}] },
        { id: 'p3', name: '789 Pine Ln', units: [{name: 'Main House', tenant: 'David Miller'}] }
    ];
    const mockUtilities = ['Water', 'Sewer', 'Trash', 'Electricity', 'Gas'];
    ...
    const createRubsForm = document.getElementById('createRubsForm');
    ...
    const updateBillTotal = (recalculateFees = true) => {
            ...
            if (tooltip) {
                tooltip.innerHTML = `
                    <div class="mb-1 border-b border-gray-600 pb-1">
                        ...
                        <div class="text-xs text-gray-400">Rule: Standard RUBS Markup</div>
                    </div>
                    <div class="mb-1 border-b border-gray-600 pb-1">
        ...
        const globalUtilityName = utilitySelect ? (utilitySelect.value || 'N/A') : 'N/A';
        ...
        if (markupTooltip) {
            ...
            rows.forEach(row => {
                ...
                if (amount > 0) {
                    ...
                    tooltipContent.push(`Standard Markup (10%) - ${globalPropertyName} - ${unitSelect.options[unitSelect.selectedIndex]?.text || 'N/A'} - ${globalUtilityName} - ${formatCurrency(itemMarkup)}`);
                }
            });
        ...
        if (discountTooltip) {
            ...
            rows.forEach(row => {
                ...
                if (amount > 0) {
                    ...
                    tooltipContent.push(`Standard Discount (5%) - ${globalPropertyName} - ${unitSelect.options[unitSelect.selectedIndex]?.text || 'N/A'} - ${globalUtilityName} - ${formatCurrency(itemDiscount)}`);
                }
            });
        ...
        if (salesTaxTooltip) {
            ...
            rows.forEach(row => {
                ...
                if (amount > 0) {
                    ...
                    totalTax += itemTax;
                    tooltipContent.push(`Standard Sales Tax (2%) - ${globalPropertyName} - ${unitSelect.options[unitSelect.selectedIndex]?.text || 'N/A'} - ${globalUtilityName} - ${formatCurrency(itemTax)}`);
                }
            });
    ...
    const createBillItemRow = () => {
        ...
        unitSelect.addEventListener('change', () => {
            ...
            if (selectedOption && selectedOption.dataset.tenant) {
                tenantInput.value = selectedOption.dataset.tenant;
            } else {
                tenantInput.value = '';
            }
        });
    ...
    if (utilitySelect) {
        utilitySelect.addEventListener('change', updateBillTotal);
    }
    ...
    if (createRubsForm) {
        createRubsForm.addEventListener('submit', (e) => {
            e.preventDefault();
            const formData = new FormData(createRubsForm);
            const billData = {
            ...
            console.log('New RUBS Bill Created (simulation):', billData);
            alert('RUBS Bill created successfully! (See console for data). Redirecting to vendor list.');
